...
bool points = intersection.Intersection(straid1, straid2);
...
TEST(LineCircleTest, TwoSolution_04)
{
  ...
  Intersections intersection;
bool points = intersection.Intersection(straid1, circle1);
...
TEST(CircleCircleTest, OneSolutions_11)
{
  ...
  Intersections intersection;
//  std::vector<Point> points = intersection.Intersection(circle1, circle2);
  bool points = intersection.Intersection(circle1, circle2);
  ASSERT_EQ(points.size(), 1);
  ASSERT_NEAR(points[0].getX(), 0.0, THE_TOLERANCE);
  ASSERT_NEAR(points[0].getY(), 0.0, THE_TOLERANCE);
}
